...
class RBACManager {
    ...
    async loadRoles() {
        try {
            const res = await apiRequest('/api/rbac/roles', { method: 'GET' });
    ...
    loadDemoData() {
        this.roles = [
            { 
                ...
                user_count: 1, permissions: ['device:view', 'device:create', 'device:edit', 'device:delete', 'data:view', 'data:export', 'alarm:view', 'alarm:manage', 'user:manage', 'system:config'],
                created_at: '2026-01-01'
            },
            { 
                id: 2, name: '设备管理员', description: '管理设备和数据', 
                user_count: 2, permissions: ['device:view', 'device:create', 'device:edit', 'data:view', 'data:export', 'alarm:view'],
                created_at: '2026-01-15'
            },
            { 
                id: 3, name: '只读用户', description: '仅查看权限', 
                user_count: 5, permissions: ['device:view', 'data:view', 'alarm:view'],
                created_at: '2026-02-01'
            },
            { 
                id: 4, name: '告警处理员', description: '处理告警和通知', 
                user_count: 3, permissions: ['device:view', 'data:view', 'alarm:view', 'alarm:manage'],
        ...
        if (!this.roles.length) {
            container.innerHTML = `
                <div class="text-center py-16 text-slate-500">
                    <i class="fas fa-user-tag text-4xl mb-3 opacity-50"></i>
                    <p class="text-lg">暂无角色</p>
                    <p class="text-sm mt-1">点击"新建角色"创建第一个角色</p>
                </div>
            `;
            return;
        }
            ...
            <div class="p-5 hover:bg-slate-700/20 transition-colors">
                <div class="flex items-start justify-between">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap gap-1.5 mb-3">
                                ${(role.permissions || []).slice(0, 6).map(p => `
                                    <span class="px-2 py-0.5 rounded text-xs bg-slate-800 text-slate-300 border border-slate-700">${this.formatPerm(p)}</span>
                                `).join('')}
                    ...
                    <div class="flex items-center gap-2 ml-4">
                        ${role.id !== 1 ? `
                            <button onclick="rbacV2.editRole(${role.id})" 
                                    class="p-2 rounded-lg hover:bg-slate-700 transition-colors" title="编辑">
                                <i class="fas fa-edit text-sm text-blue-400"></i>
                            </button>
                            <button onclick="rbacV2.deleteRole(${role.id}, '${role.name}')" 
        ...
        container.innerHTML = this.users.map(user => `
            <div class="p-5 hover:bg-slate-700/20 transition-colors">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-4">
                        <select onchange="rbacV2.assignRole(${user.id}, this.value)" 
                                class="bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-cyan-500">
                            ${this.roles.map(r => `<option value="${r.id}" ${r.name === user.role_name ? 'selected' : ''}>${r.name}</option>`).join('')}
                        </select>
                        
                        <button onclick="rbacV2.editUser(${user.id})" 
                                class="p-2 rounded-lg hover:bg-slate-700 transition-colors" title="编辑">
                            <i class="fas fa-edit text-sm text-blue-400"></i>
                        </button>
                    </div>
    ...
    async saveRole(e) {
        ...
        try {
            showNotification(id ? '角色更新成功' : '角色创建成功', 'success');
            this.closeModal();
            ...
            if (!id) {
                ...
                if (idx >= 0) Object.assign(this.roles[idx], data);
            }
    ...
    async assignRole(userId, roleId) {
        ...
        if (role && user) {
            ...
            showNotification(`已将 ${user.username} 分配到 ${role.name}`, 'success');
        }
    }
