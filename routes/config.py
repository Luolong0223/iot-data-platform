...
@config_bp.route('/configs/batch', methods=['POST'])
@login_required
def batch_get_configs():
    ...
    if not data or 'config_keys' not in data:
        return jsonify({'error': '缺少必要参数'}), 400
    
    try:
        result = ConfigService.batch_get_configs(user_id, data['config_keys'])
...
def get_groups():
    ...
    try:
        groups = ConfigService.get_config_groups(user_id)
